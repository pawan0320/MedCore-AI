    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="flex-1 bg-white rounded-xl shadow-sm border border-slate-200 p-6 min-h-[500px]">
           {successMsg && (
              <div className="mb-4 p-3 bg-emerald-50 text-emerald-700 rounded-lg flex items-center gap-2 text-sm border border-emerald-100 animate-in fade-in slide-in-from-top-2">
             ...
             <div className="space-y-6">
               <div className="flex items-center gap-4 mb-6">
                  <div className="relative">
                    <img src={formData.avatar} alt="Profile" className="w-20 h-20 rounded-full border-2 border-slate-100 object-cover" />
                    <button className="absolute bottom-0 right-0 bg-blue-600 text-white p-1 rounded-full hover:bg-blue-700 border-2 border-white">
                        <Settings size={12} />
                    </button>
                  </div>
                  <div>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="flex flex-col gap-1">
                      ...
                      <select 
                        ...
                        className="p-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none bg-white"
                      >
           ...
           {activeTab === 'MEDICAL' && user.role === Role.PATIENT && (
             <div className="space-y-6">
               <div className="bg-blue-50 p-4 rounded-lg border border-blue-100 flex items-start gap-3">
                  ...
                  <div>
                      <h4 className="font-bold text-blue-900 text-sm">HIPAA Protected Information</h4>
               ...
               <div className="space-y-6">
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                       ...
                       <div className="flex flex-col gap-1">
                           ...
                           <div className="p-2 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-lg text-sm font-semibold flex items-center gap-2">
                               <Shield size={14}/> {(formData as Doctor).verificationStatus || 'PENDING'}
                           </div>
                       </div>
                   </div>
                   <div className="p-4 border border-slate-200 rounded-lg bg-slate-50">
                       <h4 className="font-bold text-sm text-slate-700 mb-2">Upload Documents</h4>
           ...
           {activeTab === 'SECURITY' && (
             <div className="space-y-6">
                <div className="border-b border-slate-100 pb-6">
                   <h3 className="text-lg font-bold text-slate-800 mb-4">Password & Authentication</h3>
                   <div className="space-y-4 max-w-md">
                       <button className="w-full text-left px-4 py-3 border border-slate-300 rounded-lg hover:bg-slate-50 flex justify-between items-center group">
           ...
           {activeTab === 'PREFS' && (
             <div className="space-y-8">
                 <div className="space-y-4">
                     <h3 className="font-bold text-slate-800 flex items-center gap-2"><Bell size={18} /> Notifications</h3>
                 ...
                 <div className="space-y-4">
                     <h3 className="font-bold text-slate-800 flex items-center gap-2"><Globe size={18} /> Appearance & Language</h3>
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="flex flex-col gap-1">
                        ...
                        <div className="flex items-center justify-between p-3 border border-slate-200 rounded-lg">
                           <span className="text-sm text-slate-700 flex items-center gap-2"><Moon size={16} /> Dark Mode</span>
                           <label className="relative inline-flex items-center cursor-pointer">
                              <input 
                                type="checkbox" 
                                className="sr-only peer"
                 ...
                 {user.role === Role.PATIENT && (
                     <div className="pt-6 border-t border-slate-200">
                         <h3 className="font-bold text-slate-800 mb-2 flex items-center gap-2 text-red-600"><AlertCircle size={18} /> Danger Zone</h3>
                         <button className="px-4 py-2 border border-red-200 text-red-600 rounded-lg text-sm hover:bg-red-50 font-medium">
                             Delete Account
                         </button>
                     </div>
                 )}
...
const TabButton = ({ active, onClick, icon, label }: { active: boolean, onClick: () => void, icon: React.ReactNode, label: string }) => (
    <button 
      onClick={onClick}
      className={`w-full text-left px-4 py-3 rounded-lg flex items-center gap-3 transition-colors ${active ? 'bg-blue-50 text-blue-700 font-medium' : 'text-slate-600 hover:bg-slate-50'}`}
    >
        {icon} {label}
    </button>
);
...
const InputGroup = ({ label, value, onChange, type = "text", placeholder, disabled = false }: any) => (
    <div className="flex flex-col gap-1">
        ...
        <input 
            ...
            className="p-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none disabled:bg-slate-50 disabled:text-slate-500"
        />
    </div>
);
...
const TextAreaGroup = ({ label, value, onChange }: any) => (
    <div className="flex flex-col gap-1">
        ...
        <textarea 
            ...
            onChange={(e) => onChange(e.target.value)}
            rows={3}
            className="p-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none resize-none"
        />
    </div>
);
